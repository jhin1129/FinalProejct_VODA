...
ALTER TABLE PEOPLE ADD PEOPLE_STATUS VARCHAR2(1) DEFAULT 'Y';
...
ALTER TABLE CONTENTS ADD C_VIEWDATE DATE DEFAULT CURRENT_DATE NULL;
ALTER TABLE CONTENTS ADD C_VIEWCOUNT NUMBER DEFAULT 0 NULL;
...
---- CURRENT_DATE 테스트 테이블
--CREATE TABLE D_TEST (
--	D_DATE	DATE	DEFAULT CURRENT_DATE
--);
--
---- 날짜 및 시간 테스트 삽입 프로시저
--CREATE OR REPLACE PROCEDURE INSERT_D_TEST AS 
--BEGIN
--INSERT INTO D_TEST VALUES(
--    CURRENT_DATE
--    );
--END INSERT_D_TEST;
--
...
--BEGIN
--DBMS_SCHEDULER.CREATE_JOB (
--            JOB_NAME => 'JOB_INSERT_D_TEST',
--            JOB_TYPE => 'STORED_PROCEDURE',
--            JOB_ACTION => 'INSERT_D_TEST',
--            NUMBER_OF_ARGUMENTS => 0,
--            START_DATE => TRUNC(CURRENT_DATE) + 12/24,
...
--END;
--
---- 날짜 및 시간 초기화 테스트 프로시저 및 스케쥴러
--CREATE OR REPLACE PROCEDURE UPDATE_D_TEST_DATE AS 
--BEGIN
--UPDATE D_TEST SET 
--D_DATE = (
--SELECT D_DATE
--FROM D_TEST
--WHERE EXTRACT(YEAR FROM D_DATE) = EXTRACT(YEAR FROM CURRENT_DATE) AND EXTRACT(MONTH FROM D_DATE) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(DAY FROM D_DATE) = EXTRACT(DAY FROM CURRENT_DATE)
--);
--END UPDATE_D_TEST_DATE;
----TO_DATE(TO_CHAR(CURRENT_DATE+9/24, 'YYYY/MM/DD HH24:MI:SS'), 'YYYY/MM/DD HH24:MI:SS');
--BEGIN
--DBMS_SCHEDULER.CREATE_JOB (
--            JOB_NAME => 'JOB_UPDATE_D_TEST_DATE',
--            JOB_TYPE => 'STORED_PROCEDURE',
--            JOB_ACTION => 'UPDATE_D_TEST_DATE',
--            NUMBER_OF_ARGUMENTS => 0,
--            START_DATE => CURRENT_DATE,
--            REPEAT_INTERVAL => 'FREQ=MINUTELY;INTERVAL=10',
--            END_DATE => NULL,
--            ENABLED => TRUE,
--            AUTO_DROP => FALSE,
--            COMMENTS => '테스트 테이블 날짜 초기화');
--END;

--select TO_CHAR(D_DATE,'YYYYMMDDHH24MISS') as 전체날짜 FROM D_TEST;
